        ...
        private void SaveInvoice()
        {
            using (var connection = _databaseHelper.GetConnection())
            {
                ...
                using (var transaction = connection.BeginTransaction())
                {
                    try
                    {
                        ...
                        foreach (var kvp in totalQuantities)
                        {
                            ...
                            List<BatchItem> batches = GetBatchesForItem(connection, itemId);
                            int availableQuantity = batches.Sum(b => b.Quantity);
                            ...
                            foreach (var batch in batches)
                            {
                                ...
                                _batchDeductions.Add(new BatchDeduction
                                {
                                    BatchId = batch.Id,
                                    ItemId = itemId,
                ...
                VALUES (@InvoiceNumber, @ItemId, @Rate, @Quantity, @DiscountPercentage, @Total);";
                        ...
                        foreach (var item in _invoiceItems)
                        {
                            using (var command = new SQLiteCommand(insertInvoiceItemQuery, connection, transaction))
                            {
                                ...
                                command.Parameters.AddWithValue("@Rate", item.SaleRate); // Use SaleRate (selling price)
                ...
                WHERE Id = @ItemId;";
                        ...
                        foreach (var deduction in _batchDeductions)
                        {
                            ...
                            if (dbItem != null)
                            {
                                dbItem.AvailableQuantity -= deduction.Quantity;
                            }
                        }
